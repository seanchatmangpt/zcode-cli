...
export async function runGallCommand(args: string[]): Promise<number | undefined> {
  ...
  if (args[1] !== "verify") {
    console.error("Usage: zcode gall verify --bundle DIR [--json] [--out FILE]");
    return 2;
  }

  const bundle = option(args, "--bundle");
  if (!bundle) {
    console.error("Error: zcode gall verify requires --bundle DIR");
    return 2;
  }

  try {
    const receipt = verifyGallBundle(bundle);
