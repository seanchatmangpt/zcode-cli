...
import { findPartAlternatives, inspectSemanticPart, semanticCandidateFalsifier } from "../src/parts-cli.ts";
...
describe("admission refuses malformed graphs with typed errors (not TypeError)", () => {
  const cases: Array<[string, unknown, string]> = [
    ...
    ["null semantic entry", graphOf([part("a", "1", ["q"]), { ...part("b", "2", []), semantics: { algorithm: [null] } }]),
      "semantic value for b/algorithm must be an object"],
    ["axis not array", graphOf([{ ...part("a", "1", ["q"]), semantics: { algorithm: [{ semantic_id: "q" }], domain: "junk" } }]),
      "semantic axis domain for a must be an array"],
    ["numeric semantic_id", graphOf([part("a", "1", ["q"]), { ...part("c", "3", []), semantics: { algorithm: [{ semantic_id: 7 }] } }]),
      "semantic_id must be a non-empty string"],
    ["source not an object", graphOf([{ ...part("a", "1", ["q"]), source: "file:1" }]), "source must be an object"],
    ["duplicate semantic identity", graphOf([{ ...part("a", "1", []), semantics: { algorithm: [{ semantic_id: "q" }, { semantic_id: "q" }] } }]),
      "duplicate semantic identity q for a/algorithm"],
...
describe("subject resolution", () => {
  test("exact part_id outranks a colliding file_id, independent of array order", () => {
    const parts = [part("a", "b", ["q"]), part("b", "9", ["q"]), part("c", "3", ["q"])];
    expect(inspectSemanticPart(graphOf(parts), "b").part_id).toBe("b");
    expect(inspectSemanticPart(graphOf([...parts].reverse()), "b").part_id).toBe("b");
    expect(findPartAlternatives(graphOf(parts), "b").map((candidate) => candidate.part_id)).toEqual(["a", "c"]);
  });

  test("an empty selector never resolves to a part without a file_id", () => {
    const graph = graphOf([{ part_id: "a", authority: "NONE", semantics: { algorithm: [{ semantic_id: "q" }] } }]);
    expect(() => inspectSemanticPart(graph, "")).toThrow("unknown semantic-parts subject");
  });

  test("falsifying a candidate against itself is refused, not reported as a failed match", () => {
    expect(() => semanticCandidateFalsifier(good, "1", "codegraph:file:1")).toThrow("candidate must differ from subject");
  });
...
describe("axis handling", () => {
  test("duplicate axes are refused instead of inflating coverage above 1", () => {
    expect(() => findPartAlternatives(good, "1", ["algorithm", "algorithm"])).toThrow("must be unique");
    expect(() => findPartAlternatives(good, "1", ["algorithm", " algorithm "])).toThrow("must be unique");
  });

  test("inherited Object.prototype names are not axes", () => {
    for (const axis of ["toString", "constructor", "hasOwnProperty", "__proto__"]) {
      expect(() => findPartAlternatives(good, "1", [axis])).toThrow(`unsupported semantic axis ${axis}`);
    }
  });

  test("an own __proto__ semantics key is admitted as data and never pollutes Object.prototype", () => {
    const graph = JSON.parse(
      '{"schema":"unrdf.semantic-parts.v1","authority":"NONE","parts":['
      + '{"part_id":"a","authority":"NONE","semantics":{"algorithm":[{"semantic_id":"q"}],"__proto__":[{"semantic_id":"p"}]}},'
      + '{"part_id":"b","authority":"NONE","semantics":{"algorithm":[{"semantic_id":"q"}],"__proto__":[{"semantic_id":"p"}]}}]}'
    );
    const [alternative] = findPartAlternatives(graph, "a", ["algorithm"]);
    expect(alternative!.part_id).toBe("b");
    expect(Object.getPrototypeOf(alternative!.shared)).toBe(Object.prototype);
    expect(({} as Record<string, unknown>).semantic_id).toBeUndefined();
  });
...
describe("real CLI through the launcher (subprocess)", () => {
  ...
  test("receipt_digest binds the graph bytes: same query, different bytes, different receipt", () => {
    const a = writeGraph("bind-a.json", good);
    const b = writeGraph("bind-b.json", JSON.stringify(good, null, 1));
    const ra = JSON.parse(cli(["parts", "inspect", "--graph", a.path, "--subject", "1", "--json"]).stdout);
    const rb = JSON.parse(cli(["parts", "inspect", "--graph", b.path, "--subject", "1", "--json"]).stdout);
    expect(ra.part).toEqual(rb.part);
    expect(ra.graph_sha256).not.toBe(rb.graph_sha256);
    expect(ra.receipt_digest).not.toBe(rb.receipt_digest);
  });
  ...
  test("a flag is never consumed as another flag's value", () => {
    ...
    const run = cli(["parts", "alternatives", "--graph", "--subject", "1", "--json"]);
    expect(run.code).toBe(1);
    expect(JSON.parse(run.stdout)).toMatchObject({ standing: "REFUSED", detail: "--graph requires a value" });
    ...
    const swallowed = cli(["parts", "falsify", "--graph", path, "--subject", "1", "--candidate", "--json"]);
    expect(swallowed.code).toBe(1);
    expect(JSON.parse(swallowed.stdout).detail).toBe("--candidate requires a value");
  });

  test("unknown verb exits 2 with usage and no stdout", () => {
    const run = cli(["parts", "replace", "--graph", "x", "--subject", "1"]);
    expect(run.code).toBe(2);
    expect(run.stderr).toContain("zcode parts alternatives --graph FILE");
    expect(run.stdout).toBe("");
  });
