...
| `src/parts-cli.ts` (+ `src/launcher.ts` dispatch, `test/parts-cli.test.ts`, `test/parts-cli-hardening.test.ts`, `test/parts-cli-bench.test.ts`, `scripts/bench-parts-alternatives.ts`, `docs/bench/v26.9.26/parts-alternatives-bench.json`) | Thin consumer of admitted `unrdf.semantic-parts.v1` graphs for `zcode parts alternatives`; performs SELECT only, emits CANDIDATE/authority NONE, and never edits source or actuates a replacement. v26.9.26 hardening: total (query-independent) admission, ambiguous-subject refusal, distinct axes, own-property axis lookup, code-point tie-break, `graph_sha256` output + `--graph-sha256` stale-graph pin, flag-as-value refusal; seeded benchmark with a near-linear regression bound | UNSUPPORTED (generator-capability) | No pack currently projects a zcode-specific consumer CLI for the semantic-parts graph contract; matching logic is intentionally limited to exact canonical `semantic_id` intersection and refuses ambient authority |
