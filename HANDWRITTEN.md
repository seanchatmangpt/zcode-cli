...
| `src/parts-cli.ts` (+ `src/launcher.ts` dispatch, `test/parts-cli.test.ts`) | Thin consumer of admitted `unrdf.semantic-parts.v1` graphs for `zcode parts alternatives`; performs SELECT only, emits CANDIDATE/authority NONE, and never edits source or actuates a replacement | UNSUPPORTED (generator-capability) | No pack currently projects a zcode-specific consumer CLI for the semantic-parts graph contract; matching logic is intentionally limited to exact canonical `semantic_id` intersection and refuses ambient authority |
